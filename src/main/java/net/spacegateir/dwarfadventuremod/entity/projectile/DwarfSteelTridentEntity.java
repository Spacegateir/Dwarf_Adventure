package net.spacegateir.dwarfadventuremod.entity.projectile;
...
public class DwarfSteelTridentEntity extends PersistentProjectileEntity {
    private static final TrackedData<Byte> LOYALTY = DataTracker.registerData(DwarfSteelTridentEntity.class, TrackedDataHandlerRegistry.BYTE);
    private static final TrackedData<Boolean> ENCHANTED = DataTracker.registerData(DwarfSteelTridentEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    private ItemStack tridentStack;
    private boolean dealtDamage;
    public int returnTimer;

    public DwarfSteelTridentEntity(EntityType<? extends DwarfSteelTridentEntity> entityType, World world) {
        super(entityType, world);
        this.tridentStack = ItemStack.EMPTY; // Initialize as empty
    }

    public DwarfSteelTridentEntity(World world, LivingEntity owner, ItemStack stack) {
        super(TridentEntityTypes.DWARF_STEEL_TRIDENT, owner, world);
    ...
    @Override
    public void tick() {
        // Logic for returning the trident if the owner is alive
        ...
        if (loyaltyLevel > 0 && (this.dealtDamage || this.isNoClip()) && entity != null) {
            if (!this.isOwnerAlive()) {
                ...
                this.setNoClip(true);
                Vec3d eyePos = entity.getEyePos().subtract(this.getPos());
                this.setPos(this.getX(), this.getY() + eyePos.y * 0.015 * loyaltyLevel, this.getZ());
                if (this.getWorld().isClient) {
                    this.lastRenderY = this.getY();
                }
                double d = 0.05 * loyaltyLevel;
                this.setVelocity(this.getVelocity().multiply(0.95).add(eyePos.normalize().multiply(d)));
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        ...
        float damage = 10.0F; // Base damage for the Dwarf Steel Trident
        if (entity instanceof LivingEntity livingEntity) {
            damage += EnchantmentHelper.getAttackDamage(this.tridentStack, livingEntity.getGroup());
        }
    ...
    public void age() {
        int loyaltyLevel = this.dataTracker.get(LOYALTY);
        if (this.pickupType != PersistentProjectileEntity.PickupPermission.ALLOWED || loyaltyLevel <= 0) {
            super.age();
        }
    }
