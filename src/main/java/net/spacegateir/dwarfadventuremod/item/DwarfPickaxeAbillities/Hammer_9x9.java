...
public class Hammer_9x9 extends MiningToolItem {

    // Constructor
    public Hammer_9x9(ToolMaterial material, Settings settings) {
        super(2f, -3.0f, material, BlockTags.PICKAXE_MINEABLE, settings);
    }

    // Method to get a list of blocks in a 9x9 area around the initial block
    public static List<BlockPos> getBlocksToBeDestroyed(BlockPos initialBlockPos, ServerPlayerEntity player) {
        ...
        int miningRange = 4;  // 9 blocks wide in all directions (4 blocks on each side)
        int outerRange = 5;   // 11 blocks wide in all directions (5 blocks on each side)
