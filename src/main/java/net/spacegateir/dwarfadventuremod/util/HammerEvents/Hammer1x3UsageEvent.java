...
import net.spacegateir.dwarfadventuremod.item.DwarfPickaxeAbillities.Hammer_1x3;
import org.jetbrains.annotations.Nullable;

import java.util.HashSet;
import java.util.Set;

public class Hammer1x3UsageEvent implements PlayerBlockBreakEvents.Before {
    ...
    public boolean beforeBlockBreak(World world, PlayerEntity player, BlockPos pos, BlockState state, @Nullable BlockEntity blockEntity) {
        ...
        // Check if the player's tool is a Hammer with 1x3 ability and the player is on the server
        if (mainHandItem.getItem() instanceof Hammer_1x3 && player instanceof ServerPlayerEntity serverPlayer) {
            // Prevent breaking the same block multiple times
            if (HARVESTED_BLOCKS.contains(pos)) {
                return true;  // Block already handled, skip it
            }

            // Get all blocks in the 1x3 area around the initial block
            for (BlockPos position : Hammer_1x3.getBlocksToBeDestroyed(pos, serverPlayer)) {
