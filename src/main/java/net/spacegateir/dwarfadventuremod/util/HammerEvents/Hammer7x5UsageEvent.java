...
import net.spacegateir.dwarfadventuremod.item.DwarfPickaxeAbillities.Hammer_7x5;
import org.jetbrains.annotations.Nullable;

import java.util.HashSet;
import java.util.Set;

public class Hammer7x5UsageEvent implements PlayerBlockBreakEvents.Before {
    ...
    public boolean beforeBlockBreak(World world, PlayerEntity player, BlockPos pos, BlockState state, @Nullable BlockEntity blockEntity) {
        ...
        // Check if the player's tool is a Hammer with 7x5 ability and the player is on the server
        if (mainHandItem.getItem() instanceof Hammer_7x5 && player instanceof ServerPlayerEntity serverPlayer) {
            // Prevent breaking the same block multiple times
            if (HARVESTED_BLOCKS.contains(pos)) {
                return true;  // Block already handled, skip it
            }

            // Get all blocks in the 7x5 area around the initial block
            for (BlockPos position : Hammer_7x5.getBlocksToBeDestroyed(pos, serverPlayer)) {
